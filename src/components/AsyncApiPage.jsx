...
const mapStateToProps = (state) => {
    ...
    if (state && state.asyncApiSchemas && state.asyncApiSchemas.length > 0 && state.currentAsyncApiSchemaSlug) {
        for (let i = 0; i < state.asyncApiSchemas.length; i++) {
            if (state.asyncApiSchemas[i].slug === state.currentAsyncApiSchemaSlug) {
                currentAsyncApiSchema = state.asyncApiSchemas[i];
                break;
            }
        }
    ...
    return {
        currentAsyncApiSchema: currentAsyncApiSchema
    }
};
...
function AsyncApiPage(props) {
    ...
    const schema = {
        url: props.currentAsyncApiSchema.url
    };
    ...
    return (
        <Provider store={store}>
            <AsyncApiComponent schema={specMock}></AsyncApiComponent>
        </Provider>
    );
}
